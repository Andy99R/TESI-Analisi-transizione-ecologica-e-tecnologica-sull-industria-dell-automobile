---
title: "Script R - Tesi RUSU - CLEST 2"
output:
  html_document:
    df_print: paged
...
```
# ```{r Subset df ITALIA, message=FALSE, warning=FALSE}
# #Creazione DATAFRAME SOLO ITALIA
# df_ita <- filter(dataset, Paese == "Italy") %>% na.omit() 
# df_ita <- as.data.frame(df_ita)
# 
# describe.by(dataset)
# ```
# 
...
# time <- c(1995,2000,2005,2010,2015,2019)
...
# ```
# ```{r Grafico PIL, message=FALSE, warning=FALSE}
# ggplot(dataset, aes(x = Anno, y = GDP, col = Paese, group = Paese)) +
#   geom_line() +
#   xlab("Anno") +
#   ylab("Tasso PIL annuale per paese") + 
#   scale_x_discrete(breaks = time) +
#   labs(title = "Prodotto Interno Lordo paesi di riferimento")
# ```
# 
...
# ggplot(df_ita, aes(x = Registrations, y = Employment)) +
#   geom_point() +
#   xlab("Immatricolazioni nuove auto Italia") +
#   ylab("Occupazione settore auto Italia") +
#   labs(title = "Scatterplot Immatricolazioni-Occupazione") + 
#   geom_smooth(method=lm, se=FALSE,col="darkorange")
# ```
# 
# ```{r Scatter2, message=FALSE, warning=FALSE}
# ggplot(df_ita, aes(x = Registrations, y = Patents_G)) +
#   geom_point() +
#   xlab("Immatricolazioni nuove auto Italia") +
#   ylab("Brevetti green registrati in Italia") +
#   labs(title = "Scatterplot Immatricolazioni - Brevetti Green") +
#   geom_smooth(method=lm, se=FALSE, col="darkorange")
# ```
# 
# ```{r Scatter3, message=FALSE, warning=FALSE}
# ggplot(df_ita, aes(x = Patents_G, y = Employment)) +
#   geom_point(color="red") +
#   ylab("Occupazione settore auto Italia") +
#   xlab("Registrazione Brevetti green Italia") +
...
# ggplot(df_ita, aes(x = Patents_H, y = Employment)) +
#   geom_point(color="red") +
#   ylab("Occupazione settore auto Italia") +
#   xlab("Registrazione brevetti tecnologia ibrida") +
#   labs(title = "Scatterplot occupazione - Brevetti hybrid") + 
#   geom_smooth(method=lm, se=FALSE, col="deepskyblue3")
# ```
# 
# ```{r Scatter5, message=FALSE, warning=FALSE}
# ggplot(df_ita, aes(x = Patents_B, y = Employment)) +
#   geom_point(color="red") +
#   ylab("Occupazione settore auto Italia") +
#   xlab("Registrazione brevetti motore a combustione interna") +
#   labs(title = "Scatterplot occupazione - Brevetti brown") +
#   geom_smooth(method=lm, se=FALSE,col="deepskyblue3")
# ```
# 
# ```{r Scatter6, message=FALSE, warning=FALSE}
# ggplot(df_ita, aes(x = Patents_G, y = Employment_equipment)) +
#   geom_point(color="blue") +
#   ylab("Occupazione settore fornitori Italia") +
#   xlab("Registrazione brevetti green Italia") +
...
# ggplot(df_ita, aes(x = Patents_H, y = Employment_equipment)) +
#   geom_point(color="blue") +
#   ylab("Occupazione fornitori Italia") +
#   xlab("Registrazione brevetti tecnologia ibrida") +
...
# ggplot(df_ita, aes(x = Patents_B, y = Employment_equipment)) +
#   geom_point(color="blue") +
#   ylab("Occupazione settore fortnitori Italia") +
#   xlab("Registrazione brevetti motore a combustione interna") +
...
dataset$Population <- dataset$Population/10000

#Qui viene creato il modello Fixed Effects
modello_fe <- plm(Employment ~ Registrations + Patents_G + Patents_H + Patents_B + R.D + Imports + Exports + Wage + GDP + Population, data = dataset, index = c("Paese", "Anno"), model = "within")

#Qui viene creato il modello Random Effects 
modello_re <- plm(Employment ~ Registrations + Patents_G + Patents_H + Patents_B + R.D + Imports + Exports + Wage + GDP + Population, data = dataset, index = c("Paese", "Anno"), model = "random", random.method = "walhus")
...
tab_model(modello_fe,modello_re, show.aic = T,show.se = T, show.ci = F, dv.labels = c("Modello FE","Modello RE"))
...
#Questa funzione crea un nuovo dataset con le variabili esplicative laggate t-1
 lagged_dataset <- dataset %>%
   group_by(Paese) %>%
   mutate(Registrations = lag(Registrations), Patents_G = lag(Patents_G),Patents_H = lag(Patents_H),Patents_B = lag(Patents_B),R.D = lag(R.D),Exports = lag(Exports),Imports = lag(Imports),Wage=lag(Wage), GDP=lag(GDP), Population=lag(Population))
...
modello_fe_lagged <- plm(Employment ~ Registrations + Patents_G + Patents_H + Patents_B + R.D + Imports + Exports + Wage + GDP + Population, data = lagged_dataset, index = c("Paese", "Anno"), model = "within")
...
#modello_fe <- coeftest(modello_fe_lagged) #modello con gli standard errors normali

#modello_fe #Per vedere coefficienti senza standard error robusti
```
### Model for Employment_equipment

```{r Modello FE Employment fornitori, message=FALSE, warning=FALSE}
modello_fe_lagged_equipment <- plm(Employment_equipment ~ Registrations + Patents_G + Patents_H + Patents_B + R.D + Imports + Exports + Wage_equip + GDP + Population, data = lagged_dataset, index = c("Paese", "Anno"), model = "within")

R_squared_equipment <- r.squared(modello_fe_lagged_equipment) #Estraggo R^2 modello equipment

modello_fe_adjusted_equipment <- coeftest(modello_fe_lagged_equipment,vcovHC) #Questo è il modello con gli standard errors
#modello_fe_equipment <- coeftest(modello_fe_lagged_equipment) #modello con gli standard errors normali
#modello_fe_equipment #Per vedere coefficienti senza standard error robusti
models <- list("Modello automotive robusto" = modello_fe_adjusted, "Modello fornitori robusto" = modello_fe_adjusted_equipment)
